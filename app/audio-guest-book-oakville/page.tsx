import type { Metadata } from 'next';
import ServiceLocationPage, { ServiceLocationData } from '@/components/ServiceLocationPage';
import SchemaMarkup from '@/components/SchemaMarkup';
import { serviceSchema } from '@/lib/schema';
import { venues, audioGuestBookPackages } from '@/lib/sil-data';

export const metadata: Metadata = {
  title: 'Audio Guest Book Oakville | The Photobooth Guy',
  description: 'Audio guest book rental in Oakville. Vintage telephone captures voice messages at weddings. Professional service across the GTA.',
  keywords: 'audio guest book Oakville, wedding voicemail guest book, audio guestbook rental Oakville, voice message guest book, telephone guest book Oakville',
  alternates: {
    canonical: 'https://www.thephotoboothguy.ca/audio-guest-book-oakville',
  },
  openGraph: { url: 'https://www.thephotoboothguy.ca/audio-guest-book-oakville' },
};

const pageData: ServiceLocationData = {
  serviceName: 'Audio Guest Book',
  serviceSlug: 'audio-guest-book',
  city: 'Oakville',
  heroTitle: 'Audio Guest Book in Oakville',
  heroSubtitle: 'Capture heartfelt voice messages from your guests with a vintage-style telephone guest book. A unique and memorable alternative to traditional guest books for Oakville weddings and events.',
  description: 'Oakville\'s sophisticated community appreciates unique touches that make events memorable. Our audio guest book brings a charming vintage aesthetic to venues like Oakville Conference Centre and Oakville Centre for the Performing Arts while capturing something priceless - the actual voices of your loved ones. For Oakville\'s close-knit families and business communities, hearing familiar voices share stories, blessings, and well-wishes creates a treasure that written messages simply can\'t match. The vintage telephone adds an elegant decorative element that complements Oakville\'s refined venues, while the technology behind it ensures crystal-clear recording quality. The system is intuitive and self-operating, making it accessible for all your Oakville guests regardless of age, and you\'ll receive professionally edited audio files as digital downloads within 48 hours.',
  venues: venues.oakville,
  features: [
    {
      icon: '📞',
      title: 'Vintage Telephone Design',
      description: 'Audio guest book rental in Oakville. Vintage telephone captures voice messages at weddings. Professional service across the GTA.',
    },
    {
      icon: '🎙️',
      title: 'High-Quality Recording',
      description: 'Professional audio recording equipment captures clear, crisp voice messages that you\'ll treasure for years to come.',
    },
    {
      icon: '💬',
      title: 'Custom Greeting',
      description: 'Audio guest book rental in Oakville. Vintage telephone captures voice messages at weddings. Professional service across the GTA.',
    },
    {
      icon: '💾',
      title: 'Digital Delivery',
      description: 'Audio guest book rental in Oakville. Vintage telephone captures voice messages at weddings. Professional service across the GTA.',
    },
    {
      icon: '👤',
      title: 'Professional Setup',
      description: 'Our team sets up the audio guest book, tests everything, and ensures it\'s ready for your Oakville event.',
    },
    {
      icon: '♾️',
      title: 'Unlimited Messages',
      description: 'Audio guest book rental in Oakville. Vintage telephone captures voice messages at weddings. Professional service across the GTA.',
    },
  ],
  packages: audioGuestBookPackages,
  faqs: [
    {
      question: 'How much does an audio guest book cost in Oakville?',
      answer: 'Audio guest book rental in Oakville is $450 for full event coverage. The package includes the vintage telephone handset, custom recorded greeting, unlimited voice messages, easy setup, and digital audio files delivered after your Oakville event. It\'s a unique alternative to traditional guest books that captures the actual voices of your loved ones.',
    },
    {
      question: 'How does the audio guest book work at a Oakville wedding?',
      answer: 'It\'s incredibly simple! Guests pick up the vintage telephone handset, listen to your custom greeting with instructions, then leave their voice message after the beep. Just like leaving a voicemail. Our professional attendant is on site to assist anyone who needs help, but most guests figure it out right away. The system is intuitive and self-operating, so it doesn\'t require constant monitoring.',
    },
    {
      question: 'Can I listen to the audio guest book messages during my Oakville event?',
      answer: 'The audio guest book is designed to record messages throughout your Oakville event. You\'ll receive all the recordings after your event on a USB drive and as digital files, so you can listen and relive those special moments for years to come. This way you can focus on enjoying your celebration while guests leave messages throughout the day.',
    },
    {
      question: 'What happens to the audio guest book recordings after my Oakville wedding?',
      answer: 'We carefully transfer all voice messages to a high-quality USB drive and also provide you with digital audio files. You\'ll have permanent copies of every heartfelt message from your Oakville wedding or event. The recordings are never shared or kept by us after delivery - they\'re yours to keep forever and share with family and friends.',
    },
    {
      question: 'How many people typically use the audio guest book at Oakville events?',
      answer: 'Most guests love the audio guest book! Typically 60-80% of attendees leave messages at Oakville weddings and events. The vintage telephone is eye-catching and the concept is unique, which encourages participation. Many guests actually leave longer, more personal messages than they would write in a traditional guest book.',
    },
    {
      question: 'Can I use an audio guest book with a photo booth at my Oakville event?',
      answer: 'Absolutely! Many of our Oakville clients combine an audio guest book with our photo booth rental or glam booth services. It gives guests multiple ways to participate and creates different types of memories - visual photos and audio keepsakes from your special day. Both can run simultaneously without any issues.',
    },
    {
      question: 'Do you provide setup for audio guest books in Oakville?',
      answer: 'Yes! Our team sets up the audio guest book, tests everything, and ensures it\'s ready for your Oakville event. We\'ll work with your venue to find the perfect spot where guests can easily access it. The setup is quick and doesn\'t require much space, so it works well at most Oakville venues.',
    },
  ],
  relatedServices: [
    {
      name: 'Photo Booth Rental Oakville',
      href: '/photo-booth-rental-oakville',
    },
    {
      name: 'Glam Booth Oakville',
      href: '/glam-booth-oakville',
    },
    {
      name: '360 Booth Oakville',
      href: '/360-booth-oakville',
    },
  ],
};

export default function AudioGuestBookOakville() {
  return (
    <>
      <SchemaMarkup
        schema={serviceSchema(
          'Audio Guest Book Rental Oakville',
          'Audio guest book rental in Oakville with vintage telephone handset for capturing voice messages at weddings and events. Includes professional attendant, custom greeting, USB drive, and digital files.',
          '$450'
        )}
      />
      <ServiceLocationPage data={pageData} />
    </>
  );
}

