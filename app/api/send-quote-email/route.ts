...
export async function POST(request: Request) {
  try {
    ...
    const { contactInfo, serviceType = 'photo-booth', package: pkg, addons, total, notifyBusiness = true } = body;
    ...
    // Optionally notify business about the quote
    if (notifyBusiness) {
      const timestamp = formatEasternDateTime();
      const formattedEventDate = contactInfo.eventDate &&
      ...
      const businessHtml = `
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="padding: 30px;">
            <h1 style="color: #000; font-size: 24px; font-weight: 300; margin-top: 0;">Quote Sent to Client</h1>
            <p style="color: #666; font-size: 16px;">A quote was sent to a potential client through thephotoboothguy.ca.</p>
            <div style="background-color: #f9f9f9; border-left: 3px solid #000; padding: 20px; margin: 25px 0; border-radius: 4px;">
              <table style="width: 100%; border-collapse: collapse;">
                ...
                <tr><td style="padding: 8px 0; color: #666; font-size: 14px; font-weight: 600; text-transform: uppercase;">Event Date:</td><td style="padding: 8px 0;">${formattedEventDate || contactInfo.eventDate}</td></tr>
                <tr><td style="padding: 8px 0; color: #666; font-size: 14px; font-weight: 600; text-transform: uppercase;">Service:</td><td style="padding: 8px 0;">${serviceName}</td></tr>
                <tr><td style="padding: 8px 0; color: #666; font-size: 14px; font-weight: 600; text-transform: uppercase;">Package:</td><td style="padding: 8px 0;">${packageDisplay}</td></tr>
                ${addonsList.length ? `<tr><td style="padding: 8px 0; color: #666; font-size: 14px; font-weight: 600; text-transform: uppercase;">Add-ons:</td><td style="padding: 8px 0;">${addonsList.join(', ')}</td></tr>` : ''}
                <tr><td style="padding: 8px 0; color: #666; font-size: 14px; font-weight: 600; text-transform: uppercase;">Total:</td><td style="padding: 8px 0; font-weight: 600;">$${total.toLocaleString()}</td></tr>
              </table>
            </div>
      ...
      await resend.emails.send({
        ...
        subject: `Quote Sent: ${contactInfo.name} - ${serviceName} - ${timestamp}`,
        html: businessHtml,
      });
    }
